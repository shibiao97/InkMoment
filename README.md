...
提供 7 种样式可切换，每种都有 `full`（完整参数）和 `clean`（仅品牌 + 机型）两种呈现：

| 代号 | 名称 | 风格 | 是否有 clean 变体 |
| :--- | :--- | :--- | :---: |
| **A** | 富士底栏 | 白色信息条，左机型/镜头 ｜ 中品牌 Logo ｜ 右参数/时间 | — |
| **B** | 极简底栏 | 顶/左/右贴边窄白 + 底部居中 Logo + 信息 | ✓ |
| **C** | 毛玻璃悬浮 | 原图模糊作背景 + 缩小照片悬浮居中带阴影 | ✓ |
| **D** | 经典白边相框 | 顶/左/右窄白 + 底大白边居中放品牌 | ✓ |
| **F** | 杂志风 | 左下色卡（从图自动提取）+ 右下品牌信息 | ✓ |
