...
<template>
  <main class="app-shell studio-shell flow-workbench preview-shell">
    ...
    <section class="studio-main flow-main flow-main-stack">
      ...
      <header class="preview-topbar flow-hero">
        <div>
    ...
    <aside class="studio-inspector flow-inspector preview-inspector">
      <section class="inspector-card inspector-status-card">
        <p class="eyebrow">当前状态</p>
        <h2>{{ statusLabel }}</h2>
        <p>{{ groupCount.toLocaleString() }} 组 · {{ nearMinutes }} 分钟时间窗</p>
      </section>
      ...
      <section class="inspector-card inspector-stat-grid">
        ...
        <div>
          <strong>{{ nearMinutes }}</strong>
          <span>分钟时间窗</span>
        </div>
        <div>
          <strong>{{ thresholdNear }}</strong>
          <span>同场景宽容度</span>
        </div>
        <div>
          <strong>{{ thresholdFar }}</strong>
          <span>跨场景严格度</span>
        </div>
      </section>

      <section class="inspector-card preview-controls">
