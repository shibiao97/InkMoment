...
def create_folder_blueprint(get_session=None, pic_dir_factory=None, skipped_log_path_factory=None):
    ...
    def api_skipped():
        if get_session is None or skipped_log_path_factory is None:
            return jsonify({"skipped": []})
        return jsonify(read_skipped_log(get_session(), skipped_log_path_factory))

    @folder_bp.route("/api/open_folder", methods=["POST"])
    def api_open_folder():
        """跨平台打开 folder 或 _inkmoment 子目录。"""
        if get_session is None or pic_dir_factory is None:
            return jsonify({"error": "no session"}), 400
        data = request.get_json(silent=True) or {}
        payload, status = open_session_folder(get_session(), data.get("sub"), pic_dir_factory)
        return jsonify(payload), status
