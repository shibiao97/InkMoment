...
- 已迁移当前选片组读取入口、组响应序列化、跳过已完成组 helper、派发前坏图预检编排、擂台状态推进 helper、undo 快照记录、用户偏好统计、完成组落盘/应用编排、`/api/choose` 擂台选择推进、`/api/kick` 单侧淘汰、`/api/skip_group` 状态变更、`/api/undo` 快照恢复和 `/api/reopen_group` 跨组反悔编排到 `server.services.selection_service`；解码判断、skipped 记录和底层文件应用/还原 helper 暂留 `app.py` 并通过回调注入。
