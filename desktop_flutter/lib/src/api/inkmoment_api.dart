...
class InkMomentApi {
  ...
  Future<Map<String, dynamic>> redeem(String code) =>
      postJson('/api/auth/redeem', {'code': code});
  Future<Map<String, dynamic>> logout() => postJson('/api/auth/logout');
  Future<Map<String, dynamic>> unbindDevice(String reason) => postJson(
  ...
  Future<Map<String, dynamic>> depDownloadCancel() =>
      postJson('/api/dependencies/download/cancel');
  ...
  Future<Map<String, dynamic>> getAutoRejected() =>
      getJson('/api/auto_rejected');
  Future<Map<String, dynamic>> confirmPrescreen() =>
      postJson('/api/confirm_prescreen');
  Future<Map<String, dynamic>> getPreviewGroups() =>
      getJson('/api/preview_groups');
  Future<Map<String, dynamic>> getGroup() => getJson('/api/group');
  Future<Map<String, dynamic>> choose(String loser) =>
      postJson('/api/choose', {'loser': loser});
  ...
  Future<Map<String, dynamic>> previewExport(Map<String, dynamic> payload) =>
      postJson('/api/export/preview', payload);
  Future<Map<String, dynamic>> startExport(Map<String, dynamic> payload) =>
      postJson('/api/export/start', payload);
  ...
  Future<Map<String, dynamic>> openExportOutDir() =>
      postJson('/api/export/open_out_dir');

  String imageUrl(Object? path, {int width = 1200}) {
    final value = path?.toString() ?? '';
    if (value.isEmpty) return '';
    final query = Uri(
      queryParameters: {'path': value, 'w': width.toString()},
    ).query;
